...
"""
Description:
Test simple_kalman unscented kalman filter implementation
...
def twod_motion_track():
    ...
    uki_params = sk.UKIHyperParams(kappa, alpha, b)
    vx = 1. 
    vy = 2.
    F = np.identity(4)
    ...
    lam = .01
    H_func = lambda x: np.array([x[0,0], x[1,0]]).reshape(2,1) + lam*np.square(np.array([x[0,0], x[1,0]]).reshape(2,1))


    sigma_n = .5
    R = np.square(sigma_n)*np.identity(2)
    om = sk.ObservationModel(H_func, R)
    kf = sk.UnscentedKalmanFilter(dm, om, uki_params)
    N = 100
    ...
    plt.suptitle('Observations and underlying state, and estimated position')
    print(len(est_x), len(obs_x), len(true_x))

    plt.figure()
    plt.suptitle('Observations, true state and estimated state for x')
    plt.plot(est_x, 'r+')
    plt.plot(true_x, 'b.')
    plt.plot(obs_x, 'k*')
